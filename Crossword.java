...
/**
 * A recursive backtracking Crossword puzzle solving algorithm 
 * implemented using a ArrayList Dictionary.
 * 
 * Compilation: javac Crossword.java
 * Execution  : java Crossword dictionaryFile.txt crosswordBoard.txt
 * 
...
/* TODO
 *  
 * Print no solution if none found
 * 
 * Complete the JavaDocs for each method
 * 
 */
...
public class Crossword {
	...
	public Crossword(String[] args) {
		...
		try {										// Attempt to open the files and initialize the Scanners 
			dictionary = new Scanner (new FileInputStream(args[0]));
			board = new Scanner (new FileInputStream(args[1]));
		} catch (ArrayIndexOutOfBoundsException e) {
			System.err.println("Missing CLI Arguments");
			System.err.println("Usage: java Crossword dictionaryFile.txt crosswordBoard.txt");
			System.exit(1);
	...
	public void solve(int row, int col) {
		...
		if (theBoard[row][col] == SOLID_SPACE) {
			...
			if (row == size -1 && col == size -1) {
				printBoard();		//	Solution found, print board and exit
				endGame();			//	When using the MyDictionary, only one solution is required
			} else {
